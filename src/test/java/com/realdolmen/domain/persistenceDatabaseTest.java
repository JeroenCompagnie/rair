...
public class persistenceDatabaseTest extends JpaPersistenceTest {
	...
	public void testUsersAndPeristInitData() {
		 ...
		 * booking
		 */
		Flight flight = new Flight(partner, em.find(Airport.class, "JFK"), em.find(Airport.class, "JFK"), new Date(),
		...
		number = (int) (long) em
				...
				.setParameter("flightId", 1L).setParameter("seatType", Arrays.asList(SeatType.values()))
				.getSingleResult();
		...
		assertEquals(7, number);
		/**
		 * ENDtype Make flight, add 30 seats, 10 of each seattype Make
		 * bookingsofflights for 9 seats, 3 of each seattype; add it to a
		 * booking
		 */
